...
void Microphone::initialize()
{
    ...
    m_audioInfo.reset(new AudioDevice(format));
    connect(m_audioInfo.data(), &AudioDevice::update, [this]() {
        qDebug() << "level: " << m_audioInfo->level();

        emit valueChanged(m_audioInfo->level() * 100);
